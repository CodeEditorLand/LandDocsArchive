...
- Registers the [`onCommand`](/api/references/activation-events#onCommand)
  ...
  `onCommand:extension.helloWorld`, so the extension becomes activated when user
...
- The `postinstall` script: This would install the 1.25 version of VS Code API
  as specified in `engines.vscode`. Once the `vscode.d.ts` file is downloaded to
  `node_modules/vscode/vscode.d.ts`, you will get IntelliSense, jump to
